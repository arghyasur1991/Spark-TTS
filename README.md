...
We now provide scripts to export Spark-TTS models to ONNX format for deployment on different platforms. Currently, the following components can be exported successfully:

- BiCodec Encoder 
- BiCodec Factorized VQ Tokenize/Detokenize
- BiCodec Speaker Encoder Tokenize/Detokenize (dummy implementation)
- BiCodec Wave Generator

The LLM export is still a work in progress, but you can use the exported BiCodec components with a Python-based LLM inference pipeline.

To export models to ONNX format:

```sh
PYTHONPATH=$PYTHONPATH:. python cli/convert_models_to_onnx.py --model_name Spark-TTS-0.5B
```

This will create ONNX models in the `onnx_models/Spark-TTS-0.5B/` directory.

To run inference with the exported ONNX models:

```sh
PYTHONPATH=$PYTHONPATH:. python cli/run_onnx_inference.py --text "Text to synthesize." \
  --onnx_model_dir onnx_models/Spark-TTS-0.5B \
  --llm_tokenizer_dir pretrained_models/Spark-TTS-0.5B/LLM \
  --bicodec_config pretrained_models/Spark-TTS-0.5B/BiCodec/config.yaml \
  --gender female --pitch moderate --speed moderate \
  --output_wav output.wav
```

You can also compare PyTorch and ONNX outputs using:

```sh
PYTHONPATH=$PYTHONPATH:. python cli/test_onnx_vs_pytorch.py \
  --text "Text to synthesize." \
  --pytorch_model_dir pretrained_models/Spark-TTS-0.5B \
  --onnx_model_dir onnx_models/Spark-TTS-0.5B \
  --llm_tokenizer_dir pretrained_models/Spark-TTS-0.5B/LLM \
  --bicodec_config pretrained_models/Spark-TTS-0.5B/BiCodec/config.yaml \
  --gender female --pitch moderate --speed moderate \
  --output_pytorch_wav pytorch_output.wav \
  --output_onnx_wav onnx_output.wav
```

Note: Since the LLM cannot currently be exported to ONNX, the inference scripts use dummy semantic tokens. For full functionality, you'll need to use the PyTorch LLM model to generate tokens, then pass those tokens to the ONNX BiCodec components.
